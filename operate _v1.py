...
class Operate:
    ...
    def update_slam(self, drive_meas):
        ...
        #lms = yolo_pose_est(lms, self.img)
        if self.request_recover_robot:
            ...
            if self.command['inference'] == True:
                self.detect_target()
                self.estimate_pose()
                for target, position in self.target_pose_dict.items():
                    x,y = position['x'],position['y']
                    xx,depth = position['xx'],position['depth']
                    if target=='apple':
                        for pos_x,pos_y,tag in zip(self.ekf.markers[0][11:14],self.ekf.markers[1][11:14], self.ekf.taglist[11:14]):
                            dist = ((pos_x - x)**2 + (pos_y-y)**2)**0.5
                            if dist<0.1:
                                lms_tag = tag
                                print("Class: {}".format(target))
                                print("Object Tag: {}".format(lms_tag))
                                print("Object Estimated Location: {},{}".format(round(x,2),round(y,2)))
                                print("Matching from map: {},{}".format(round(pos_x,2),round(pos_y,2)))
                                lms.append(measure.Marker(np.array([depth,xx]), lms_tag))
                    if target=="lemon":
                        for pos_x,pos_y,tag in zip(self.ekf.markers[0][14:17],self.ekf.markers[1][14:17], self.ekf.taglist[14:17]):
                            dist = ((pos_x - x)**2 + (pos_y-y)**2)**0.5
                            if dist<0.1:
                                lms_tag = tag
                                print("Class: {}".format(target))
                                ...
                                lms.append(measure.Marker(np.array([depth,xx]), lms_tag))
                    if target=='person':
                        for pos_x,pos_y,tag in zip(self.ekf.markers[0][17:],self.ekf.markers[1][17:], self.ekf.taglist[17:]):
                            dist = ((pos_x - x)**2 + (pos_y-y)**2)**0.5
                            if dist<0.1:
                                lms_tag = tag
                                print("Class: {}".format(target))
    ...
    def detect_target(self):
        if self.command['inference'] and self.detector is not None:
            ...
            #self.detector_output = self.detector.detect_single_image(img320)
    ...
    def estimate_pose(self):
        ...
        for target_indx in range(len(detections)):
            ...
            target_pose = {'y':0, 'x': 0, 'xx':0, 'depth':0}
            ...
            target_pose['xx'] = x
            target_pose['depth'] = depth
            # print("ROBOT POSE  {}".format(robot_pose))
            # print("Target: {}".format(target_list[target]))
            # print(target_pose)
            self.target_pose_dict[target_list[target]] = target_pose
    ...
    #run path planning
    def pathplanning(self, new_path, robot_pose, step, current_path, len_path, path_no, to_move, reached_goal):
        if self.command['autopathplan'] == True:
            
            if new_path:
                print("Planning path")
                #define apple, lemon, person list
                apple_list, lemon_list, person_list, _,_  = parse_map("map1.txt")#change map name?
                to_move = compute_dist(apple_list, lemon_list, person_list)
                print(to_move)
                new_path = 0
                step = 0
                path_no = 0
                obstacle_list = create_obstacle_list(mapname = "map1.txt")
                #edit current_path to move
                current_path = planPath(to_move[path_no], robot_pose, apple_gt, lemon_gt, person_gt, obstacle_list)
                print(current_path)
                len_path = len(current_path)

            self.drive_flag = 1
            self.drive_to_point(current_path[step], robot_pose)
            num_to_move = len(to_move)
            if reached_goal:
                step = step + 1
            if step == len_path:
                path_no = path_no + 1
                if path_no < num_to_move:
                    obstacle_list = create_obstacle_list(mapname = "map1.txt")
                    current_path = planPath(to_move[path_no], robot_pose, apple_gt, lemon_gt, person_gt, obstacle_list)
                    print(current_path)
                    len_path = len(current_path)
                    ...
                    step = 0
            return new_path, current_path, step, len_path, path_no, to_move, current_path[step], 1
        return new_path, current_path, step, len_path, path_no, to_move, 0, 0
        ...
        if self.drive_flag:
            ...
            if abs(robot_pose[0] - waypoint[0]) > THRESHOLD or abs(robot_pose[1] - waypoint[1]) > THRESHOLD:
                ...
                if (turn_vel>0.15)and(turn_vel<0.3):
                    ...
                    turn_vel = int(round(err_ang*P_t))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        operate.drive_to_point(waypoint, operate.ekf.robot.state)
        operate.take_pic()
        ...
        if operate.command['autopathplan'] == True:
            new_path, current_path, step, len_path, path_no, to_move, waypoint, operate.drive_flag = operate.pathplanning(new_path, 
                                operate.ekf.robot.state, step, current_path, len_path, path_no, to_move, operate.reached_goal_flag)
            new_path, operate.drive_flag = operate.stopplanning(new_path)
        # visualise
        ...
        if event == '-GRAPH-':
            if mouse == (None, None):
                continue
            waypoint = [mouse[0]/g_size*3-1.5, mouse[1]/g_size*3-1.5]
            print(waypoint)
        ...
        #     print("Inference is set to: ".format(operate.command['inference']))
        # if inference:
        #     time_elapsed = time.time() - operate.last_time
        #     if time_elapsed > 2:
        #         operate.command['inference'] = True
        #         operate.last_time = time.time()
        #     else:
