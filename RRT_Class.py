...
class RRT:
    ...
    def planning(self, animation=True):
        ...
        while len(self.node_list) <= self.max_nodes:
            ...
            # ---------------- TODO: Complete the main steps of the RRT algorithm ---------------------------
    ...
    def is_collision_free(self, new_node):
        ...
        if new_node is None:
            return True
        #print([new_node.path_y[a][0] for a in range(len(new_node.path_x))])
        #points = np.ndarray([[new_node.path_x[a], new_node.path_y[a]] for a in range(len(new_node.path_x))]).T
