...
class Circle:
    def __init__(self, centre, radius): #c_x, c_y, radius):
        self.centre = np.array(centre) #np.array([c_x, c_y])
        self.radius = radius
    ...
    def is_in_collision_with_points(self, points):
        ...
        dist = []
        for point in points:
            dx = self.centre[0] - point[0]
            dy = self.centre[1] - point[1]
...
class Rectangle(Polygon):
    ...
    def __init_(self, origin=np.zeros, width=100, height=20):
        self.width = width
        self.height = height
        self.origin = origin

        v1 = origin + np.array([width/2, height/2]) #tl
        v2 = origin + np.array([-width/2, height/2])#tr
        v3 = origin + np.array([width/2, -height/2])#bl
        v4 = origin + np.array([-width/2, -height/2])#br
