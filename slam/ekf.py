...
class EKF:
    ...
    def predict_covariance(self, raw_drive_meas):
        ...
        if raw_drive_meas.left_speed == 0 and raw_drive_meas.right_speed == 0 and self.robot.state[0] == 0 and self.robot.state[1] == 0 and self.robot.state[2] == 0:
            #not moving
            Q = Q
            # Q += 0.05
        elif raw_drive_meas.left_speed == -raw_drive_meas.right_speed:
            #rotating
            Q[0,0] += (8.2383e-6)/20/10.98
            Q[1,1] += (8.2383e-6)/20/10.98
            Q[2,2] += 0.1194152/5/18.73
            # Q[0:3,0:3] += 0.1194152 * 5 * np.eye(3)
        elif raw_drive_meas.left_speed == raw_drive_meas.right_speed:
            #moving forward
            Q[0,0] += (8.2383e-6)/20/10.98
            Q[1,1] += (8.2383e-6)/20/10.98
            # Q[0,0] += (8.2383e-6)*20* np.eye(3)
        else: # Moving and rotating
            Q[1,1] += (8.2383e-6)/20/10.98
            Q[0,0] += (8.2383e-6)/20/10.98
            Q[2,2] += 0.1194152/5/18.73
            # Q[0:3,0:3] += 0.1194152 * 5 * np.eye(3)
    ...
    def add_landmarks(self, measurements):
        ...
        for lm in measurements:
            ...
            #z: testing reduced covariance
            self.P[-2,-2] = self.init_lm_cov
            self.P[-1,-1] = self.init_lm_cov
            #print(self.markers)
            #print(self.taglist)
